...
let tags = {
  ...
  try {
    ...
    let after = conn.menu.after || (conn.user.jid == await Connection.conn?.user?.jid ? '' : `Powered by https://wa.me/${await Connection.conn.user.jid.split`@`[0]}`) + defaultMenu.after
    let _text = [
      before,
...
handler.tags = ['main']
handler.command = /^(allmenu|all|menuall)$/i
