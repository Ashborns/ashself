...
const handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  db.data.dbai.netwrck[m.sender] = db.data.dbai.netwrck[m.sender] || { model: "openai/gpt-4o-mini" };
  ...
  if (command === "netwrck") {
    ...
    await conn.reply(m.chat, "🔄 *Proses sedang berlangsung, harap tunggu...*", m);
    try {
  ...
  if (command === "netwrckimg") {
    ...
    await conn.reply(m.chat, "🔄 *Proses menghasilkan gambar, harap tunggu...*", m);
    try {
      ...
      m.reply("❌ *Terjadi kesalahan selama pemrosesan gambar.*");
    }
  }
};

handler.before = async (m, { conn }) => {
  db.data.dbai = db.data.dbai || {};
  db.data.dbai.netwrck = db.data.dbai.netwrck || {};
  db.data.dbai.netwrck[m.sender] = db.data.dbai.netwrck[m.sender] || { model: "openai/gpt-4o-mini" };
